...
class pyparsing_test:
    ...
    class reset_pyparsing_context:
        ...
        def restore(self):
            ...
            if self._save_context["default_whitespace"] != ParserElement.DEFAULT_WHITE_CHARS:
    ...
    def with_line_numbers(
        ...
        if mark_control is not None:
            if mark_control == "unicode":
                tbl = str.maketrans(
                    {c: u for c, u in zip(range(0, 33), range(0x2400, 0x2433))} | {127: 0x2421}
                )
                eol_mark = ""
            else:
                tbl = str.maketrans(dict.fromkeys(list(range(0, 32)) + [127], mark_control))
            s = s.translate(tbl)
